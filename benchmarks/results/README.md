...
| `l20-sparse-repetition-penalty-serving/` | L20 serving runner smoke | Proves the paired native-vs-custom serving runner starts vLLM, sends opt-in logits-processor requests, and records trace events. The checked-in Qwen3-0.6B smoke is `smoke_only=true` because the tiny batch-one shape stays outside the sparse gate (`candidate_sparse_op_hits=0`), so it is not a latency claim. |
