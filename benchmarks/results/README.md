...
| `cpu-real-model/` | CPU real-model smoke | Adds a non-mock CPU baseline using `llama-cpp-python` and SmolLM2-135M-Instruct Q4_K_M GGUF with `n_gpu_layers=0`: 17 prompt tokens, 16 decode tokens, 4 CPU threads, 4.742771 ms median decode step, and 209.868062 decode tok/s on the checked-in local smoke. |
