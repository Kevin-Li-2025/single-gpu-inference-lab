...
| `l20-sparse-repetition-penalty/` | L20 standalone CUDA boundary | Compares a dense full-vocabulary repetition-penalty pass against a custom sparse history-token CUDA kernel: 39 correct cases, 1.26x median speedup, up to 4.09x on throughput-batched Qwen-size vocabularies; the measured dispatch policy chooses sparse for 21/39 cases with zero regressions. |
