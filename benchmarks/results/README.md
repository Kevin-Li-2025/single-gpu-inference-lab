...
| `cpu-real-model/` | CPU real-model smoke | Adds a non-mock CPU baseline using SmolLM2-135M-Instruct Q4_K_M GGUF with `n_gpu_layers=0`: the Python-call-path smoke reaches 209.868062 decode tok/s, while the standard `llama-bench` control reports `pp17` 596.351643 tok/s, `tg16` 359.429002 tok/s, and `pp17+tg16` 412.212899 tok/s. The cached Qwen2.5-Coder-0.5B GGUF is recorded as invalid local cache, not a benchmark. |
