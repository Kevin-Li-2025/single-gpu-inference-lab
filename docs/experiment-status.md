...
| CPU real GGUF model | Smoke | `scripts/benchmark_cpu_real_model.py`, `scripts/bench_cpu_real_model.sh`, `scripts/bench_cpu_llama_bench.sh`, `scripts/summarize_cpu_llama_bench.py`, `benchmarks/results/cpu-real-model/`, `docs/cpu-small-model-boundary.md` | Non-mock CPU baseline on SmolLM2-135M-Instruct Q4_K_M GGUF with `n_gpu_layers=0`. Python-call-path smoke: 17 prompt tokens, 16 decode tokens, 4 CPU threads, 4.742771 ms median decode step, 209.868 decode tok/s. Standard `llama-bench` on the same GGUF: `pp17` 596.351643 tok/s, `tg16` 359.429002 tok/s, `pp17+tg16` 412.212899 tok/s. Qwen2.5-Coder-0.5B is not benchmarked because the local cache file is invalid GGUF (`00000000` magic instead of `GGUF`). |
