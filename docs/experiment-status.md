...
| Apple M4 Q4_K affine SME2 | Positive real-tensor kernels / negative full decode | `cpp/m4_q4k_sme2.cpp`, `integrations/llama_cpp/kevin_m4_q4k_sme2.h`, `docs/m4-q4k-sme2-case-study.md`, `benchmarks/results/cpu-m4-q4k-sme2/qwen25-coder-3b-affine-v1/` | The self-written Q4_K-preserving affine transform reaches `1.132x` and `1.158x` over custom raw NEON on two full Qwen 3B FFN tensors, with `1e-7` mapping NRMSE. Persistent llama.cpp integration and a 1-SME/3-x8 hybrid keep greedy output byte-identical, but `tg128` median falls from 33.98 to 29.13 tok/s and p95/p99 ITL regresses. Keep disabled; the layer win does not beat four-thread llama x8 end to end. |
