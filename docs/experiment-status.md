...
| L20 sparse repetition penalty | Positive standalone CUDA boundary / serving scaffold | `benchmarks/results/l20-sparse-repetition-penalty/`, `integrations/vllm/l20_sparse_repetition_penalty_logits_processor.py` | Dense full-vocabulary repetition-penalty scans are avoidable on Qwen-size vocabularies and throughput batches: 39 correct L20 cases, 1.26x median speedup, up to 4.09x. The measured dispatch policy chooses sparse for 21/39 cases with zero regressions. The official vLLM custom logits-processor plus `l20_stack::sparse_repetition_penalty_out` dispatcher scaffold is now present, but there is still no L20 serving A/B claim. |
