...
| L20 sparse repetition penalty | Positive standalone CUDA boundary / negative first serving A/B | `benchmarks/results/l20-sparse-repetition-penalty/`, `benchmarks/results/l20-sparse-repetition-penalty-serving/`, `integrations/vllm/l20_sparse_repetition_penalty_logits_processor.py` | Dense full-vocabulary repetition-penalty scans are avoidable on Qwen-size vocabularies and throughput batches: 39 correct L20 cases, 1.26x median speedup, up to 4.09x. The measured dispatch policy chooses sparse for 21/39 cases with zero regressions. The official vLLM custom logits-processor reaches real serving (`sparse_op=65` on Qwen3-0.6B c8/i512/o32), but the standalone processor regresses median ITL 14.33 ms -> 15.67 ms; the next target is fused sampler or LM-head epilogue, not this standalone path. |
...
logits-processor scaffold -> true GEMM
