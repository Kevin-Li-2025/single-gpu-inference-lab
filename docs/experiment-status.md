...
| Apple M4 Q4_K affine SME2 | Positive real-tensor kernels / negative full decode / optimized dispatch pending AC gate | `cpp/m4_q4k_sme2.cpp`, `integrations/llama_cpp/kevin_m4_q4k_sme2.h`, `scripts/run_m4_q4k_sme2_ab.py`, `docs/m4-q4k-sme2-case-study.md`, `benchmarks/results/cpu-m4-q4k-sme2/qwen25-coder-3b-affine-v1/` | The self-written Q4_K-preserving affine transform reaches `1.132x` and `1.158x` over custom raw NEON on two full Qwen 3B FFN tensors, with `1e-7` mapping NRMSE. The original all-FFN hybrid keeps greedy output byte-identical but drops `tg128` from 33.98 to 29.13 tok/s. The follow-up defaults to down-only SME2, one shared Q8 pack, 25% SME rows, and fallback-worker affine correction overlapped with SME2; its real prompt remains byte-identical, but a formal AC/high-performance A/B is still required. Keep disabled until the power-qualified runner passes. |
...
| L20 sparse repetition penalty | Positive standalone CUDA boundary / negative standalone serving A/B / positive fused sampler matrix | `benchmarks/results/l20-sparse-repetition-penalty/`, `benchmarks/results/l20-sparse-repetition-penalty-serving/`, `benchmarks/results/l20-vllm-fused-sparse-sampling/`, `benchmarks/results/l20-sparse-penalty-triangle/`, `benchmarks/results/l20-sparse-penalty-triangle-matrix/`, `docs/l20-sparse-penalty-case-study.md` | Dense full-vocabulary repetition-penalty scans are avoidable on Qwen-size vocabularies and throughput batches: 39 correct L20 cases, 1.26x median speedup, up to 4.09x. The official vLLM custom logits-processor reaches real serving (`sparse_op=65` on Qwen3-0.6B c8/i512/o32), but regresses median ITL 14.33 ms -> 15.67 ms. Moving the same idea into the fused sampler boundary first gives a small positive smoke signal (2.609 ms -> 2.575 ms median ITL, 48/50 trace-eligible sampler events), then the formal Qwen3-0.6B triangle matrix keeps fused median ITL positive in 4/4 comparable rows and fused median E2E positive in 4/4 rows. The standalone request-level processor wins only 1/4 matrix rows, so it remains a control path, not an optimization path. |
...
logits-processor scaffold -> negative standalone serving A/B ->
fused sampler smoke -> sparse-penalty triangle matrix positive fused rows ->
