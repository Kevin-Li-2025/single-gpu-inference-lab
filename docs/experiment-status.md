...
| CPU real GGUF model | Smoke | `scripts/benchmark_cpu_real_model.py`, `scripts/bench_cpu_real_model.sh`, `benchmarks/results/cpu-real-model/`, `docs/cpu-small-model-boundary.md` | Non-mock CPU baseline using `llama-cpp-python` with `n_gpu_layers=0` on SmolLM2-135M-Instruct Q4_K_M GGUF. Local smoke: 17 prompt tokens, 16 decode tokens, 4 CPU threads, 4.742771 ms median decode step, 209.868 decode tok/s. Qwen2.5-Coder-0.5B GGUF downloaded but did not load with the installed binding, so Qwen CPU parity is a follow-up after refreshing llama.cpp. |
