...
| L20 sparse repetition penalty | Positive standalone CUDA boundary | `benchmarks/results/l20-sparse-repetition-penalty/` | Dense full-vocabulary repetition-penalty scans are avoidable on Qwen-size vocabularies and throughput batches: 39 correct L20 cases, 1.26x median speedup, up to 4.09x. The measured dispatch policy chooses sparse for 21/39 cases with zero regressions, while batch-one rows remain launch-bound; this is still a fusion target rather than a serving claim. |
...
standalone L20 CUDA sparse repetition-penalty boundary -> true GEMM
