...
| Combined sparse sampling + fused top-logprobs | Positive A100 serving A/B | `benchmarks/results/a100-vllm-combined-sampling-logprobs/` | The richer top-k/top-p + penalties + generated-token logprobs workload now wins in real vLLM HTTP serving. Against the FlashInfer sampler baseline, median ITL improves 4.406 ms -> 4.248 ms (-3.60%) and total request time 217.4 ms -> 208.2 ms (-4.25%). Against the native PyTorch sampler baseline, median ITL improves 4.549 ms -> 4.308 ms (-5.28%). Trace runs prove top-logprobs 60/60 and sparse sampler 60/62 path coverage; latency uses separate no-trace runs. |
...
top-logprobs A100 serving win -> producer-side sparse-penalty LM-head negative
proof -> true GEMM epilogue/upstream LM-head boundary
```
