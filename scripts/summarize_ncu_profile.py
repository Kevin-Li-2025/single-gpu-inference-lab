...
METRIC_ALIASES = {
    ...
    "dram_gbps": ["dram__bytes.sum.per_second", "dram__bytes.avg.per_second"],
    ...
    "l2_sector_hit_rate_pct": ["lts__t_sector_hit_rate.pct"],
...
def _read_non_comment_rows(path: Path):
    with path.open(newline="", encoding="utf-8-sig") as handle:
        return [row for row in csv.reader(line for line in handle if not line.startswith("#"))]


def _read_long_ncu_csv(rows):
    rows_by_kernel = defaultdict(dict)
    header = rows[0]
    for row_values in rows[1:]:
        row = {
            name: row_values[index] if index < len(row_values) else ""
...
def _looks_like_metric_column(name: str) -> bool:
    key = (name or "").strip()
    return key.startswith(METRIC_PREFIXES)


def _read_wide_ncu_csv(rows):
    rows_by_kernel = defaultdict(dict)
    header = rows[0]
    units = rows[1] if len(rows) > 1 else []
    unit_by_column = {
        name: units[index] if index < len(units) else "" for index, name in enumerate(header)
    }
    for row_values in rows[1:]:
        row = {name: row_values[index] if index < len(row_values) else "" for index, name in enumerate(header)}
        kernel = row.get("Kernel Name") or row.get("Kernel") or row.get("Name")
        if not kernel:
            continue
        for metric_name, raw_value in row.items():
            if not _looks_like_metric_column(metric_name):
                continue
            value = parse_number(raw_value)
            if value is None:
                continue
            rows_by_kernel[kernel][metric_name.strip()] = normalize_unit(
                value,
                unit_by_column.get(metric_name, ""),
                metric_name.strip(),
            )
    return rows_by_kernel
...
def read_ncu_csv(path: Path):
    rows = _read_non_comment_rows(path)
    if not rows:
        return {}
    header = rows[0]
    if "Metric Name" in header and ("Metric Value" in header or "Value" in header):
        return _read_long_ncu_csv(rows)
    return _read_wide_ncu_csv(rows)
...
def summarize_kernel(kernel, metrics):
    ...
    if extracted["dram_gbps"] is not None:
        achieved_gbps = extracted["dram_gbps"]
    elif duration_ns and dram_bytes is not None:
