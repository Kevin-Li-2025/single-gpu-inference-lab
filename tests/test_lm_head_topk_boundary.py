...
def test_lm_head_top1_batch_block_policy():
    assert lm_head_top1_batch_block(1) == 1
    assert lm_head_top1_batch_block(2) == 4
    assert lm_head_top1_batch_block(4) == 4
    with pytest.raises(ValueError):
        lm_head_top1_batch_block(0)
...
def test_lm_head_top1_source_is_available():
    ...
    assert "_lm_head_top1_batched_partial_kernel" in source
    assert "BLOCK_BATCH" in source
    assert "lm_head_top1_batch_block" in source
    assert "lm_head_top1_policy_config" in source
